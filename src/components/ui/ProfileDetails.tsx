"use client";

import NoResults from "./NoResults";
import PostCard from "./PostCard";
import Image from "next/image";

import { useEffect, useState } from "react";
import { Video } from "../../../types";
import { GoVerified } from "react-icons/go";

interface IProps {
  profile: any;
}

const ProfileDetails = ({ profile }: IProps) => {
  const [showUserVideos, setShowUserVideos] = useState<Boolean>(true);
  const [videosList, setVideosList] = useState<Video[]>([]);

  const { user, userVideos, userLikedVideos } = profile;
  
  const videos = showUserVideos
    ? "border-b-2 border-black text-copy font-bold"
    : "text-copy-lighter font-light";
  const liked = !showUserVideos
    ? "border-b-2 border-black text-copy font-bold"
    : "text-copy-lighter font-light";

  useEffect(() => {
    if (showUserVideos) {
      setVideosList(userVideos);
    } else {
      setVideosList(userLikedVideos);
    }
  }, [showUserVideos]);

  return (
    <div className="w-full">
      <div className="flex items-center gap-6 md:gap-10 mb-4 w-full">
        <div className="w-16 h-16 md:w-32 md:h-32">
          <Image
            width={120}
            height={120}
            layout="responsive"
            className="rounded-full"
            src={user.image}
            alt="user-profile"
          />
        </div>

        <div>
          <div className="text-copy text-md md:text-2xl font-bold tracking-wider flex gap-2 items-center justify-center lowercase">
            <span>{user.userName.replace(/\s+/g, "")} </span>
            <GoVerified className="text-blue-400 md:text-xl text-md" />
          </div>
          <p className="text-copy-light text-sm font-medium">
            {" "}
            {user.userName}
          </p>
        </div>
      </div>

      <div>
        <div className="flex gap-10 mb-10 mt-10 py-2 border-b-2 border-gray-200 w-full">
          <p
            className={`text-xl font-semibold cursor-pointer ${videos} mt-2`}
            onClick={() => setShowUserVideos(true)}
          >
            Videos
          </p>
          <p
            className={`text-xl font-semibold cursor-pointer ${liked} mt-2`}
            onClick={() => setShowUserVideos(false)}
          >
            Liked
          </p>
        </div>
        <div className="grid grid-cols-3 gap-5">
          {videosList.length > 0 ? (
            videosList.map((post: Video, idx: number) => (
              <PostCard key={idx} postDetails={post} />
            ))
          ) : (
            <NoResults
              text={`No ${showUserVideos ? "" : "Liked"} Videos Yet`}
            />
          )}
        </div>
      </div>
    </div>
  );
};

export default ProfileDetails;
